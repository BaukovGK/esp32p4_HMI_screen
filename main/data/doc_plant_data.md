...
| `DIRTY_CONTROLLER` | 11 | Статус доступности контроллера (controller_online) |
| `DIRTY_SETTINGS` | 12 | Уставки (установки параметров) |
| `DIRTY_ALL` | 0x1FFF | Все группы (биты 0-12) |
...
Каждый сеттер самостоятельно захватывает мьютекс с `pdMS_TO_TICKS(50)` через helper `plant_data_try_lock()`. При таймауте — логируется `ESP_LOGW`, пакет отбрасывается. На успех записывает данные, устанавливает dirty-бит, обновляет `last_msg_time_us` и освобождает мьютекс.
...
| `plant_data_set_controller_online(online)` | DIRTY_CONTROLLER | Онлайн-статус контроллера (доступность основного устройства) |
...
- Сеттеры используют `pdMS_TO_TICKS(50)` — конечный таймаут через helper `plant_data_try_lock()`
- Lock UI-стороны также использует конечный таймаут для предотвращения зависания
- На timeout — `ESP_LOGW` + drop пакета (для setter) либо fallback значение (для getter)
