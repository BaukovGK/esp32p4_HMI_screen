 ...
 * Прототип использует SVG viewBox 900×530. Контентная зона на железе
 * 1280×692. Aspect-fit: scale = min(1280/900, 692/530) = min(1.422, 1.306).
 * Берём меньший = 1.306 — высота заполняется, по бокам letterbox.
 * Все координаты из proto/index.html нужно пропускать через MNEMO_PX().
 *
 * Целые пиксели → fixed-point: 1306/1000.
 */
#define MNEMO_SVG_W       900
#define MNEMO_SVG_H       530
#define MNEMO_SCALE_NUM   1306
...
#define MNEMO_PX_W        MNEMO_PX(MNEMO_SVG_W)   /* ≈ 1175 */
#define MNEMO_PX_H        MNEMO_PX(MNEMO_SVG_H)   /* ≈ 692 */
