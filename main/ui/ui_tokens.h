...
/* ───── мнемосхема: scale-to-fit с прототипа в физические пиксели ───
 * Прототип использует SVG viewBox 900×530. На главном экране мы
 * делим контент с правой панелью (320 px) — мнемосхеме остаётся
 * примерно 940 px по ширине. Aspect-fit: scale = min(940/900, 668/530) =
 * min(1.044, 1.260) = 1.044. Использую 1:1 (scale = 1.0) для
 * простоты — мнемосхема 900×530 пикселей помещается в 940×668 с
 * небольшими margin'ами. Координаты из proto/index.html используются
 * напрямую через MNEMO_PX (которая теперь — identity).
 */
#define MNEMO_SVG_W       900
#define MNEMO_SVG_H       530
#define MNEMO_SCALE_NUM   1000
#define MNEMO_SCALE_DEN   1000
#define MNEMO_PX(svg)     (((int)(svg) * MNEMO_SCALE_NUM) / MNEMO_SCALE_DEN)
/* Размер мнемо-холста в физических пикселях. */
#define MNEMO_PX_W        MNEMO_PX(MNEMO_SVG_W)   /* 900 */
#define MNEMO_PX_H        MNEMO_PX(MNEMO_SVG_H)   /* 530 */
